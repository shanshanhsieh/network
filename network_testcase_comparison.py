...
percent_d_qloss = delta_qloss[0:8760].div(simulink_network_qloss[0:8760])

# delta_ploss = cea_network_dP.sub(simulink_network_dP, axis=0)
delta_ploss = simulink_network_dP.sub(cea_network_dP, axis=0)
percent_d_ploss = delta_ploss[0:8760].div(simulink_network_dP[0:8760])
...
line1, = ax1.plot(x, cea_network_dP[0:8760], label='cea')
line2, = ax1.plot(x, simulink_network_dP[0:8760], label='simulink')
ax1.legend(loc='lower right')

line3, = ax2.plot(x, percent_d_ploss[0:8760], label='d_ploss')
...
for edge in range(simulink_network_qloss.shape[1]):
        ...
        line3, = axs[edge].plot(x, cea_network_qloss[0:8760][column])
        line4, = axs[edge].plot(x, simulink_network_qloss[0:8760][column])
...
for edge in range(simulink_network_qloss.shape[1]):
        ...
        column = simulink_network_qloss.columns[edge]
        line5, = axs_1[edge].plot(x, percent_d_qloss[0:8760][column])
        #axs[edge].legend(loc='lower right')
